...
void usb_handle_buffer_status() {
    ...
    do {
        ...
        if (endpoint_buffer_status & 0x1) {
            ...
            DEBUG_TEXT = "Buffer Status Handler \tPico > Host, Processing Endpoint Number %d, IRQ=%s";
            DEBUG_SHOW ("IRQ", 1, endpoint_buffer_number, endpoint_irq_pending(endpoint_buffer_status));
        ...
        if (endpoint_buffer_status & 0x2) {
            ...
            DEBUG_TEXT = "Buffer Status Handler \tHost > Pico, Processing Endpoint Number %d, IRQ=%s";
            DEBUG_SHOW ("IRQ", 1, endpoint_buffer_number, endpoint_irq_pending(endpoint_buffer_status));
...
void isr_usbctrl() {           // USB interrupt handler IRQ5
    ...
    if (IRQ_STATUS & USB_INTS_SETUP_REQ_BITS) {
        ...
        DEBUG_TEXT = "USB Device Setup\t\tSetup Packet Received From Host, Pico Address=%d";
        DEBUG_SHOW ("IRQ", 5, get_device_address());
    ...
    if (IRQ_STATUS ^ IRQ_HANDLED) {
        ...
        DEBUG_SHOW ("IRQ", 9, (uint32_t) (IRQ_STATUS ^ IRQ_HANDLED));
        busy_wait_ms(1000);
