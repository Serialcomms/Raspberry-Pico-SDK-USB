...
struct pico_profile pico;

static uint8_t show_device_text[40];

void reset_usb_device() {

    pico.usb.device.cdc_state = false;
    
    pico.usb.device.address_set = false;
    pico.usb.descriptors.device = false;
    pico.usb.descriptors.config = false;

    pico.usb.functions.cdc.framing.baud = (uint32_t) 115200;
    pico.usb.functions.cdc.framing.format = 0;
    pico.usb.functions.cdc.framing.parity = 0;
    pico.usb.functions.cdc.framing.data = 8;

    pico.usb.functions.joystick.set_idle = false;
    pico.usb.functions.joystick.hid_report = false;

    //pico.usb.device.last_transaction.stage = 0;
    ...
    pico.usb.device.control_transfer_stage = 0;
...
void show_device_enumerated() {
    ...
    DEBUG_SHOW ("DEV", 1, pico.usb.device.address_set ? "Y" : "N"); 

    DEBUG_TEXT = "Device Enumeration\tDEVICE DESCRIPTOR SENT \t\t = %s";
    DEBUG_SHOW ("DEV", 1, pico.usb.descriptors.device ? "Y" : "N");

    DEBUG_TEXT = "Device Enumeration\tDEVICE CONFIGURATION SENT \t = %s";
    DEBUG_SHOW ("DEV", 1, pico.usb.descriptors.config ? "Y" : "N"); 

    DEBUG_TEXT = "Device Enumeration\tHID REPORT DESCRIPTOR SENT \t = %s";
    DEBUG_SHOW ("DEV", 1, pico.usb.functions.joystick.hid_report ? "Y" : "N");

    DEBUG_TEXT = "Device Enumeration\tHID SET IDLE RECEIVED \t\t = %s";
    DEBUG_SHOW ("DEV", 1, pico.usb.functions.joystick.set_idle ? "Y" : "N");

    DEBUG_TEXT = "Device Enumeration\tENUMERATION COMPLETE \t\t = %s";
    DEBUG_SHOW ("DEV", 1, usb_device_enumerated() ? "TRUE" : "FALSE");
...
void set_binary_declarations() {     
    ...
    bi_decl(bi_program_feature("Debug Probe / UART Debug"));
